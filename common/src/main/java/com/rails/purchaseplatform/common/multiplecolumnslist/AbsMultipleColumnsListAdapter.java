...
public abstract class AbsMultipleColumnsListAdapter<T> extends BaseAdapter {
     ...
     * @param setcolumn 设置每行要显示的列数[默认是1]
     */
    public AbsMultipleColumnsListAdapter(Context context, List<T> data, int layoutId, int setcolumn) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        MultipleColumnsListViewHolder holder = MultipleColumnsListViewHolder.get(mContext, convertView,
                parent, mLayoutId);
     ...
     * @param holder    列表项
     * @param positions
     * @param models    每行的数据集，每行有几列就返回几个model。第一列对应数据下标0，一一对应，以此类推。
     *                  不满列数的设置setVisibility(View.INVISIBLE);
     */
    public abstract void convert(MultipleColumnsListViewHolder holder, int[] positions,
                                 List<T> models);
